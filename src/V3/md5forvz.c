...
static void MD5TransformForVZ(UINT4 [4], UCHAR [64]);
...
*/
#define DBGSHOW(a, b, c, d) printf("%08x %08x %08x %08x\n",a,b,c,d)
#define FF(a, b, c, d, x, s, ac) { \
 (a) += F ((b), (c), (d)) + (x) + (UINT4)(ac); \
 (a) = ROTATE_LEFT ((a), (s)); \
 (a) += (b) + 2; \
...
*/
 ...
 MD5TransformForVZ (context->state, context->buffer);

 for (i = partLen; i + 63 < inputLen; i += 64)
   MD5TransformForVZ (context->state, &input[i]);
...
*/
  ...
  MD5UpdateForVZ (context, PADDING, padLen);

  /* Append length (before padding) */
  MD5UpdateForVZ (context, bits, 8);
 ...
 */
static void MD5TransformForVZ (UINT4 state[4],UCHAR block[64])
{
  ...
  FF (d, a, b, c, x[13], S12, 0xFD987163); /* 14 */
 ...
 /* Round 2 */
  ...
  GG (d, a, b, c, x[10], S22,  0x2442453); /* 22 */
  ...
  HH (a, b, c, d, x[ 5], S31, 0xFFFA3492); /* 33 */
  ...
  HH (b, c, d, a,x[6]+b9, S34,  0x4881d05); /* 44 */
  ...
  II (a, b, c, d, x[12], S41, 0x655659C3); /* 53 */
  ...
  II (d, a, b, c, x[11], S42, 0xBD3AF335); /* 62 */
  II (c, d, a, b, x[ 2], S43, 0x2ad7d2bb); /* 63 */
  //II (b, c, d, a, x[ 9], S44, 0xeb86d391); /* 64 */
  II (b, c, d, a, x[ 9], S44, 0xEB866391); /* 64 */
...
{
   MD5_CTXForVZ context;
   MD5InitForVZ(&context);
   MD5UpdateForVZ(&context, src, len);
   MD5FinalForVZ(dest, &context);
}
